...
int my_pthread_join(my_pthread_t thread, void **value_ptr) {
	...
	int found = 0;
#ifndef MLFQ
	/*If it is in run queue, run scheduler until target has finished*/
	if(searchQueue(runQueue[7], thread) == 0){
		while(runQueue[7]->end->tid != thread) raise(SIGALRM);
		raise(SIGALRM);
		found = 1;
	}
#else
	int i;
	for(i = 7; i >= 0; i--){
		if(searchQueue(runQueue[i], thread) == 0){
			while(runQueue[i]->end->tid != thread){
				raise(SIGALRM);
				if(runQueue[i]->end->tid == thread){
					raise(SIGALRM);
					found = 1;
					break;
				}
...
#endif
	if(!found){
		/*Thread has already finished, return*/
		if(searchQueue(doneList[0], thread) == 0){
			tcb *target;
			sigprocmask(SIG_BLOCK, &set, NULL);
			while(doneList[0]->end->tid != thread){
				target = dequeue(doneList[0]);
				enqueue(doneList[0], target);
			}
			target = dequeue(doneList[0]);
			free(target);
			target = NULL;
		...
		else if(searchQueue(blockList[0], thread) == 0){
			...
			while(runQueue[level]->end->tid != thread){
				raise(SIGALRM);
				if(runQueue[level] == NULL){
					level = getHighestLevel();
				}
				if(runQueue[level]->end->tid == thread){
					raise(SIGALRM);
					break;
				}
			}
...
void sched_stcf(){
	...
	// YOUR CODE HERE
	ucontext_t oldctx = newctx->context;
	printf("In STCF\n");
	sigprocmask(SIG_BLOCK, &set, NULL);
...
//		setcontext(&Main);
	...
	swapcontext(&oldctx, &newctx->context);
...
static void switchToScheduler(int signum, siginfo_t *info, void *old){
	...
	swapcontext(&newctx->context, &schedContext);
	return;
}
