...
void my_pthread_exit(void *value_ptr) {
	...
	free(thread->context);
	thread->status = STOPPED; //probably redundant since thread has already been killed
	thread = NULL;
...
int my_pthread_join(my_pthread_t thread, void **value_ptr) {
	...
	while(runQueue->end->tid != (my_pthread_t*)thread){
	...
	/*Threads have finished running, deallocate memory*/
...
static void schedule() {
	...
	// YOUR CODE HERE
	//TODO choose another thread to run
	//TODO write signal handler to catch SIGALRM when timer goes off to swap in a different thread
	//TODO use swapcontext to change contexts between the current thread(dequeued) and the new thread
	//[thread 3:READY] -> [thread 2:READY] -> [thread 1:RUNNING]
	//scheduler
	//old = dequeue = thread 1
	//new = queue->end = thread 2
	...
	tcb *new = dequeue(runQueue);
	old->context->uc_link = new->context;
	new->context->uc_link = runQueue->end->context; // this makes the front of the queue the current running thread and the end of the queue the successor
	old->status = READY;
	new->status = RUNNING;
