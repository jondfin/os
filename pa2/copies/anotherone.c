...
int my_pthread_join(my_pthread_t thread, void **value_ptr) {
	...
	tcb *hold = dequeue(runQueue[level]); //hold the calling thread until target terminates
	sigprocmask(SIG_UNBLOCK, &set, NULL);
	...
	finished = NULL;
#ifndef MLFQ
	priorityEnqueue(runQueue[7], hold);
#else	
	priorityEnqueue(runQueue[hold->priority], hold);
#endif
