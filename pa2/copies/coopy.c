...
static volatile int counter = 0, first = 0;
/* create a new thread */
...
int my_pthread_join(my_pthread_t thread, void **value_ptr) {
	...
	/*is blocked?*/
		...
		my_pthread_yield(); //just force it to the end of queue
		sigprocmask(SIG_UNBLOCK, &set, NULL);
	}
		/*is ready?*/
	int level = getHighestLevel();
	while(level != -1){
		if(searchQueue(runQueue[level], thread) == 0){
			printf("Running\n");
			break;
		}
		level--;
	}
	if(level == -1) return -1; //not found
